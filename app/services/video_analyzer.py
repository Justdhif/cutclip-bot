...
class VideoAnalyzer:
    ...
    def transcribe_trimmed_clip(self, clip_path: str) -> str:
        """
        Transcribes a short local clip directly using Groq Whisper.
        """
        try:
            ...
            logger.error(f"Gagal mentranskripsi klip kustom: {e}")
            return ""

    def analyze_custom_clip_potential(self, transcript_text: str, start_time: int, end_time: int) -> str:
        """
        Analyzes a specific custom trimmed clip transcript for its potential and adds feedback.
        """
        if not transcript_text.strip():
            transcript_text = "(Tidak ada suara percakapan terdeteksi dalam klip ini)"

        prompt = (
            f"Analisis cuplikan video kustom berikut (Rentang Waktu: {start_time} - {end_time} detik):\n"
            f"Transkrip Cuplikan:\n\"\"\"\n{transcript_text}\n\"\"\"\n\n"
            "Tuliskan laporan analisis viralitas singkat khusus untuk klip ini dengan format Markdown Indonesia:\n\n"
            "📈 **ANALISIS KLIP KUSTOM**\n"
            "• **Skor Viral**: [Nilai 0-100]/100\n"
            "• **Topik Utama**: [Topik singkat yang dibahas]\n"
            "• **Alasan Potensial**: [Alasan singkat kenapa bagian ini menarik jika diposting ke Shorts/TikTok]\n"
            "• **Rekomendasi Hook**: [Saran kalimat pembuka atau tulisan text overlay di detik 0-3 untuk memikat penonton]\n\n"
            "PENTING: Selalu sisipkan emoji standar serta emoji ASCII/Kaomoji khas Jepang (seperti (๑•̀ㅂ•́)و✧, (✿◠‿◠)) di sela-sela kalimat."
        )
        try:
            completion = self.client.chat.completions.create(
                model=self.manager.llm_model,
            ...
            return f"Gagal menganalisis klip: {str(e)}"
